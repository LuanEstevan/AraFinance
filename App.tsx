  ...
  if (!user) return (
    <div style={{ minHeight:"100vh", background:C.bg, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"32px 24px", color:C.text }}>
      <style>{`
        ...
        .auth-box { animation: fadeUp 0.3s ease both; }
        button { -webkit-tap-highlight-color: transparent; }
      `}</style>
      <div className="auth-box" style={{ width:"100%", maxWidth:380, display:"flex", flexDirection:"column", alignItems:"center" }}>
        <AraLogo size={80} id="login" />
        <div style={{ fontSize:28, fontWeight:800, color:C.text, marginBottom:4, marginTop:16 }}>Ara Finance</div>
        <div style={{ fontSize:14, color:C.sub, marginBottom:40 }}>Controle com clareza. Viva melhor.</div>

        <div style={{ display:"flex", background:C.card, borderRadius:12, padding:4, marginBottom:24, width:"100%" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12, width:"100%" }}>
          ...
          <input style={iStyle} placeholder="Senha" type="password" value={authPassword} onChange={e => setAuthPassword(e.target.value)} onKeyDown={e => e.key === "Enter" && (authView === "login" ? signIn() : signUp())} />
