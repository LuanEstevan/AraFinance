...
function TabIcon({ tab, active, blue, sub }) {
  ...
  if (tab==="lancamentos") return <svg width={w} height={h} viewBox={vb} fill="none" stroke={c} strokeWidth={sw} strokeLinecap={sl} strokeLinejoin={sj}><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="12" x2="15" y2="14"/></svg>;
  if (tab==="historico")   return <svg width={w} height={h} viewBox={vb} fill="none" stroke={c} strokeWidth={sw} strokeLinecap={sl} strokeLinejoin={sj}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="8" y1="13" x2="16" y2="13"/><line x1="8" y1="17" x2="16" y2="17"/><line x1="8" y1="9" x2="10" y2="9"/></svg>;
...
const TABS = ["dashboard","contas","lancamentos","historico","metas"];
const TAB_LABELS = { dashboard:"Dashboard", contas:"Contas", lancamentos:"Lancamentos", historico:"Historico", metas:"Metas" };
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await window.storage.get("financas-data");
        if (r && r.value) {
      ...
      try {
        const r2 = localStorage.getItem("financas-data");
        if (r2) {
  ...
  // Empty templates
  const EMPTY_TX  = useCallback(() => ({ type:"expense", description:"", amount:"", category:"Alimentacao", accountId:"", date:todayStr, installments:1 }), [todayStr]);
  ...
  const importData = () => {
    try {
      ...
      if (!d.transactions||!d.accounts) { setBackupMsg("Dados invalidos."); return; }
    ...
    } catch(e) { setBackupMsg("Texto invalido. Cole exatamente o que foi exportado."); }
  };
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, color:C.text, fontFamily:"system-ui, sans-serif", paddingBottom:80 }}>
      ...
      <div style={{ padding:"20px 20px 16px", background:"linear-gradient(180deg, #0d1424 0%, "+C.bg+" 100%)" }}>
        ...
        {tab==="dashboard" && (
          <div style={{ marginTop:20, textAlign:"center" }}>
            <div style={{ fontSize:12, color:C.sub, marginBottom:6 }}>SALDO DO MES</div>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end" }}>
              ...
              {tab==="contas" && accounts.length>0 && (
                <div style={{ textAlign:"right" }}>
                  <div style={{ fontSize:11, color:C.sub }}>{banks.length} banco{banks.length!==1?"s":""} - {cards.length} {cards.length===1?"cartao":"cartoes"}</div>
              ...
              {tab==="historico" && (
                <div style={{ textAlign:"right" }}>
                  <div style={{ fontSize:11, color:C.sub }}>{sorted.length} lancamentos</div>
      ...
      <div style={{ position:"fixed", bottom:0, left:0, right:0, background:C.surface, borderTop:"1px solid "+C.border, display:"flex", zIndex:50, paddingBottom:8 }}>
        {TABS.map(t => {
          const active = tab===t;
          const labels = { dashboard:"Dashboard", contas:"Contas", lancamentos:"Lancamentos", historico:"Historico", metas:"Metas" };
          return (
      ...
      <div style={{ padding:"16px 16px 0" }}>
          ...
          <div>
            {byCategory.length===0 && (
              <div style={{ textAlign:"center", padding:"60px 20px", color:C.sub, display:"flex", flexDirection:"column", alignItems:"center" }}>
                ...
                <div style={{ fontSize:13 }}>Adicione seu primeiro lancamento</div>
              </div>
            )}
            {byCategory.length>0 && (
            ...
            {byDate.length>0 && (
              <div style={{ background:C.card, borderRadius:16, padding:20, marginBottom:12 }}>
                <div style={{ fontSize:13, fontWeight:600, color:C.sub, marginBottom:16, textTransform:"uppercase", letterSpacing:1 }}>Resumo Diario</div>
            ...
            {(()=>{
              ...
              const data = months6.map(ym=>{ const txs=transactions.filter(t=>t.date.slice(0,7)===ym); const lbl=MONTHS[parseInt(ym.split("-")[1])-1].slice(0,3); const full=MONTHS[parseInt(ym.split("-")[1])-1]+" "+ym.split("-")[0]; return { mes:lbl, fullMes:full, receita:txs.filter(t=>t.type==="income").reduce((s,t)=>s+t.amount,0), gasto:txs.filter(t=>t.type==="expense").reduce((s,t)=>s+t.amount,0) }; });
          ...
          <div>
            {banks.length>0||cards.length>0 ? (
              <div style={{ display:"flex", gap:10, marginBottom:16 }}>
                ...
                {cards.length>0 && (
                  <div style={{ flex:1, background:C.card, borderRadius:14, padding:"14px 16px" }}>
                    <div style={{ fontSize:11, color:C.sub, marginBottom:4 }}>Fatura do Mes</div>
                      ...
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                          ...
                          <div>
                            ...
                            <div style={{ fontSize:11, color:C.sub, marginTop:2 }}>Gasto este mes: <span style={{ color:C.red }}>{fmt(spendByAccount[a.id]||0)}</span></div>
                          </div>
                        </div>
                        <div style={{ textAlign:"right" }}>
            ...
            {cards.length>0 && (
              <div style={{ marginBottom:16 }}>
                <div style={{ fontSize:11, fontWeight:600, color:C.sub, textTransform:"uppercase", letterSpacing:1, marginBottom:10 }}>Cartoes</div>
                {cards.map(a => {
                        ...
                        <div style={{ flex:1 }}>
                          ...
                          {limit>0 ? (
                            <div>
                              ...
                              <div style={{ fontSize:10, color:C.sub, marginTop:4 }}>{pct.toFixed(0)}% comprometido - <span style={{ color:available>=0?C.green:C.red }}>{fmt(available)} disponivel</span>{futureBill>0&&<span style={{ color:C.muted }}> (incl. {fmt(futureBill)} futuras)</span>}</div>
                            </div>
                          ) : (
            ...
            {accounts.length===0 && (
              <div style={{ textAlign:"center", padding:"60px 20px", color:C.sub, display:"flex", flexDirection:"column", alignItems:"center" }}>
                ...
                <div style={{ fontSize:13 }}>Adicione seu banco ou cartao</div>
              </div>
            )}
            <button onClick={openNewAcc} style={btn("#f1f5f9")}>Adicionar Banco ou Cartao</button>
          </div>
        )}
        ...
        {tab==="lancamentos" && (
          <div>
            <button onClick={openNewTx} style={btn("#f1f5f9",{ marginBottom:12 })}>Novo Lancamento</button>
            <div style={{ background:C.card, borderRadius:14, padding:16, marginBottom:16 }}>
              <div style={{ fontSize:12, color:C.sub, lineHeight:1.7, display:"flex", gap:10, alignItems:"flex-start" }}>
                ...
                <span>Vincule lancamentos a um <b style={{ color:C.text }}>banco</b> para atualizar o saldo automaticamente, ou a um <b style={{ color:C.text }}>cartao</b> para controlar a fatura.</span>
              </div>
            </div>
            {sorted.length>0 && (
              <div>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
                  <div style={{ fontSize:11, fontWeight:600, color:C.sub, textTransform:"uppercase", letterSpacing:1 }}>Ultimos lancamentos</div>
                  <button onClick={()=>setTab("historico")} style={{ background:"none", border:"none", color:C.blue, cursor:"pointer", fontSize:12, fontWeight:500 }}>Ver todos</button>
                </div>
                ...
                {sorted.length>5 && (
                  <button onClick={()=>setTab("historico")} style={{ width:"100%", background:"none", border:"1px solid "+C.border, color:C.sub, borderRadius:12, padding:"10px", fontSize:13, cursor:"pointer", marginTop:4 }}>
                    Ver mais {sorted.length-5} lancamentos
                  </button>
                )}
              </div>
            )}
            {sorted.length===0 && (
              <div style={{ textAlign:"center", padding:"40px 20px", color:C.sub, display:"flex", flexDirection:"column", alignItems:"center" }}>
                ...
                <div style={{ fontSize:14, color:C.text, marginBottom:4 }}>Nenhum lancamento ainda</div>
                <div style={{ fontSize:12 }}>Toque em "Novo Lancamento" para comecar</div>
              </div>
            )}
          </div>
        )}
        ...
        {tab==="historico" && (
          <div>
            <div style={{ position:"relative", marginBottom:14 }}>
              ...
              <input style={{ ...iStyle, paddingLeft:38, paddingRight:searchQuery?38:14 }} placeholder="Buscar lancamento..." value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} />
            ...
            <div style={{ fontSize:12, color:C.sub, marginBottom:14 }}>
              {searchQuery ? displayList.length+" resultado"+(displayList.length!==1?"s":"")+" para \""+searchQuery+"\"" : monthLabel(selectedMonth)+" - "+sorted.length+" lancamentos"}
            </div>
            ...
            {sorted.length===0&&!searchQuery && (
              <div style={{ textAlign:"center", padding:"60px 20px", color:C.sub, display:"flex", flexDirection:"column", alignItems:"center" }}>
                ...
                <div style={{ fontSize:16, fontWeight:600, color:C.text, marginBottom:8 }}>Nenhum lancamento</div>
                <div>Ainda nao ha movimentacoes neste mes.</div>
              </div>
            )}
      ...
      {/* MODAL: Lancamento */}
      {txModal && (
        <Modal title={txModal.editId!=null?"Editar Lancamento":"Novo Lancamento"} onClose={()=>setTxModal(null)}>
          <div style={{ display:"flex", gap:8, marginBottom:16, background:C.card, borderRadius:12, padding:4 }}>
            {["expense","income"].map(tp => (
              <button key={tp} onClick={()=>setTxModal(f=>({ ...f, type:tp, category:tp==="expense"?"Alimentacao":"Salario" }))} style={{ flex:1, padding:"10px", borderRadius:10, border:"none", cursor:"pointer", fontSize:13, fontWeight:600, background:txModal.type===tp?(tp==="expense"?C.red:C.green):"transparent", color:txModal.type===tp?"#fff":C.sub, display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
                {tp==="expense"
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
              ...
              <div style={{ background:C.card, borderRadius:12, padding:14 }}>
                ...
                <div style={{ display:"flex", gap:10, alignItems:"center" }}>
                  ...
                  <div style={{ flex:1, textAlign:"center", color:C.text, fontWeight:700, fontSize:15 }}>
                    {(parseInt(txModal.installments)||1)===1?"A vista":(parseInt(txModal.installments)||1)+"x de "+fmt(parseBR(txModal.amount)/(parseInt(txModal.installments)||1))}
                  </div>
            ...
            <select style={iStyle} value={txModal.accountId} onChange={e=>setTxModal(f=>({ ...f, accountId:e.target.value }))}>
              <option value="">Sem vinculo de conta</option>
              {banks.map(a=><option key={a.id} value={a.id}>Banco - {a.name}</option>)}
              {cards.map(a=><option key={a.id} value={a.id}>Cartao - {a.name}</option>)}
            </select>
            <div>
            ...
            <button onClick={saveTx} style={btn("#f1f5f9")}>{txModal.editId!=null?"Salvar alteracoes":"Adicionar lancamento"}</button>
            {txModal.editId!=null && <button onClick={async()=>{ await deleteTx(txModal.editId); }} style={btn("none",{ border:"1px solid "+C.red+"44", color:C.red })}>Remover lancamento</button>}
          </div>
        </Modal>
      )}
      ...
      {accModal && (
        <Modal title={accModal.editId!=null?"Editar Conta":"Nova Conta"} onClose={()=>setAccModal(null)}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            ...
            <button onClick={saveAcc} style={btn("#f1f5f9")}>{accModal.editId!=null?"Salvar alteracoes":"Adicionar"}</button>
      ...
      {/* MODAL: Configuracoes */}
      {showBackup && (
        <Modal title="Configuracoes" onClose={()=>{ setShowBackup(false); setBackupText(""); setBackupMsg(""); setImportText(""); setImportSuccess(false); }}>
              ...
              <div style={{ padding:"14px 16px", borderBottom:"1px solid "+C.border }}>
                <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:12 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:12, color:C.sub, marginTop:1 }}>Gere o codigo e salve no Notas do iPhone</div>
                  </div>
                </div>
                <button onClick={exportData} style={btn("linear-gradient(135deg,#10b981,#059669)",{ marginBottom:backupText?10:0 })}>Gerar codigo de backup</button>
              ...
              <div style={{ padding:"14px 16px" }}>
                <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:12 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:12, color:C.sub, marginTop:1 }}>Cole o codigo para restaurar seus dados</div>
                  </div>
                </div>
                <textarea style={{ ...iStyle, height:70, resize:"none", fontSize:16, marginBottom:10 }} placeholder="Cole seu codigo de backup aqui..." value={importText} onChange={e=>setImportText(e.target.value)} />
          ...
          <div>
            ...
            <div style={{ background:C.card, borderRadius:14, overflow:"hidden" }}>
              ...
              <div style={{ padding:"12px 16px", borderBottom:"1px solid "+C.border, display:"flex", justifyContent:"space-between" }}>
                <span style={{ fontSize:13, color:C.sub }}>Inicio do projeto</span>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            ...
            <div>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                {[
                  { value:"economia",  label:"Economia",             desc:"Guardar uma quantia este mes" },
                  { value:"gasto_max", label:"Limite de gastos",     desc:"Nao gastar mais que um valor" },
            ...
            <button onClick={()=>saveGoal(goalModal)} style={btn("#f1f5f9")}>{goalModal.editId!=null?"Salvar alteracoes":"Criar meta"}</button>
          ...
          <Modal title="" onClose={()=>setAccDetail(null)}>
            <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:20, paddingBottom:16, borderBottom:"1px solid "+C.border }}>
              ...
              <div style={{ flex:1 }}>
                ...
                <div style={{ fontSize:12, color:C.sub, marginTop:2 }}>{accDetail.kind==="bank"?"Conta bancaria":"Cartao de credito"} - {monthLabel(selectedMonth)} - {monthTxs.length} lancamentos</div>
              </div>
              ...
              {accDetail.kind==="card" && parseBR(accDetail.limit)>0 && (()=>{
                ...
                return (
                  <div style={{ flex:1, background:C.card, borderRadius:12, padding:"12px 14px", borderTop:"3px solid "+(available>=0?C.blue:C.red) }}>
                    <div style={{ fontSize:11, color:C.sub }}>Limite disponivel</div>
            ...
            {monthTxs.length===0 && (
              <div style={{ textAlign:"center", padding:"30px 0", color:C.sub, display:"flex", flexDirection:"column", alignItems:"center" }}>
                ...
                <div>Nenhum lancamento neste mes.</div>
              </div>
            )}
